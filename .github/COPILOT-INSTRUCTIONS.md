...
**Repository policy:** Copilot MUST NOT execute any `.py` scripts in this repository during interactive assistance. All check runs, assessments, and rewrites are **simulated** and should be represented by in-repo artifacts (for example `story/checks/last_full_run_simulated.txt`, `story/continuity/*.jsonc`, `story/rewrites/*.md`). If a real run is required, the user will explicitly request it and will run it locally; do not attempt to execute local scripts or remote commands on the user's machine.
